...
const OrderItemCard = ({ order }) => {
  ...
  return (
    <div className='flex flex-col md:flex-row justify-between gap-2 p-2 m-2 border border-slate-300 rounded-lg'>
      ...
      <div className='flex gap-2'>
        ...
        <div className='flex flex-col px-2 py-1 gap-1 rounded-lg text-[14px] font-semibold'>
          ...
          <p>Price - ₹{parseFloat(order.shownPrice)}</p>
          <p>Quantity - {order.quantity}</p>
          <p>Toatl Price - ₹{totalPrice}</p>
        </div>
      </div>

      <div className='flex flex-col gap-2 items-center border justify-between border-orange-200 p-2 rounded-lg'>
